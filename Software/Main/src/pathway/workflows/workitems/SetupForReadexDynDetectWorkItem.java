...
/*
 * 1. Add the SCOREP_PROFILING_FORMAT and SCOREP_METRIC_PAPI environment variables to the bullxmpi tool in experiments.
 * 2. Remove existing scorep-xyz directories in application directory.
 * 3. Return updated experiments.
 *  */
public class SetupForReadexDynDetectWorkItem extends PathwayWorkItem {
    @Override
    public Map<String, Object> execute(WorkItem workItem, WorkflowInstanceData data) {
        ...
        try {
          ...
          //  String scorePFilteringFileEnvVarName = "SCOREP_FILTERING_FILE";
        	String scorePProfilingFormat = (String) workItem.getParameter("ScorePProfilingFormat");
        	String scorePProfilingFormatEnvVarName = "SCOREP_PROFILING_FORMAT";        	
        	String scorePMetricPapi = (String) workItem.getParameter("ScorePMetricPapi");
        	String scorePMetricPapiEnvVarName = "SCOREP_METRIC_PAPI";   
        	...
        	reqEnvVarForExperimentMap.put(scorePProfilingFormatEnvVarName, scorePProfilingFormat); //set SCOREP_PROFILING_FORMAT to experiment
        	reqEnvVarForExperimentMap.put(scorePMetricPapiEnvVarName, scorePMetricPapi); // add SCOREP_METRIC_PAPI to experiment.
            ...
            //reqEnvVarsMap.remove(scorePFilteringFileEnvVarName); // remove SCOREP_FILTERING_FILE environment variable that was added to bullxmpi.
            //reqEnvVarsMap.put(scorePProfilingFormatEnvVarName, scorePProfilingFormat); //add SCOREP_PROFILING_FORMAT to bullxmpi,
            //reqEnvVarsMap.put(scorePMetricPapiEnvVarName, scorePMetricPapi); // add SCOREP_METRIC_PAPI to bullxmpi.
            ...
            impl(batchSystemManagerObj, "rm -rf scorep-*", codeLocation);
        	results.put("Experiment", experiment);
        }
        catch( Exception ex ) {
            UIUtils.showErrorMessage("Setup for readex-dyn-detect failed. The return value is non-zero. See the console log for more details.");
    ...
    private static void impl(BatchSystemManager batchSystemManagerObj, String command, String codeLocation)
        ...
        ExecutionLog.get().writeLog("Preparing script for SetupForReadexDynDetect...");
        ...
        ExecutionLog.get().writeLog("Executing script for SetupForReadexDynDetect...");
        ...
        if( output.status != 0 ) {
            ExecutionLog.get().writeErr("Script execution failed for SetupForReadexDynDetect!");
            ExecutionLog.get().writeErr(output.stdout);
            ExecutionLog.get().writeErr(output.stderr);
            throw new WorkflowErrorException();
        }

        ExecutionLog.get().writeLog("Script executed successfully for SetupForReadexDynDetect.");
        FileUtils.deleteResource(connection, scriptFile);
    }
